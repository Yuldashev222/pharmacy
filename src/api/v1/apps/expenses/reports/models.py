...
from api.v1.apps.companies.reports.models import AllReturnProductReportMonth, AllDiscountProductReportMonth
...
class DiscountProductReportMonth(models.Model):
    pharmacy = models.ForeignKey('pharmacies.Pharmacy', on_delete=models.PROTECT)
    year = models.IntegerField()
    month = models.IntegerField()
    price = models.IntegerField(default=0)
    ...
    def save(self, *args, **kwargs):
        super().save(*args, **kwargs)
        price = DiscountProductReportMonth.objects.filter(month=self.month).aggregate(s=models.Sum('price'))['s']
        obj = AllDiscountProductReportMonth.objects.get_or_create(
            year=self.year,
            month=self.month,
            director_id=self.director_id
        )[0]
