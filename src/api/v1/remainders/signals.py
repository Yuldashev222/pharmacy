...
def update_user_income_report(instance, *args, **kwargs):
    try:
        obj = RemainderDetail.objects.filter(report_date=instance.report_date,
                                             shift=instance.shift, pharmacy=instance.pharmacy).first()
        if obj:
            obj.save()
        else:
            RemainderDetail.objects.create(report_date=instance.report_date,
                                           shift=instance.shift, pharmacy=instance.pharmacy)
